...
async function updateUserAddress(
  ...
  const existingUser = userAddresses.find(
    (user) => user.address === userId && user.chainId === chainId,
  );
...
function cleanOldData() {
  ...
  userAddresses = userAddresses.filter(
    (user) => currentTime - user.lastUpdate <= 30000,
  );
  bidAsks = bidAsks.filter(
    (bidAsk) => currentTime - bidAsk.lastUpdate <= 30000,
  );
}
...
async function answerRFQ(rfqReq: RfqResponse): Promise<QuoteRequest> {
  ...
  const userAddress = userAddresses.find(
    (user) => user.address === rfqReq.userId && user.chainId === rfqReq.chainId,
  );
  if (!userAddress) {
  ...
  const configParametersLong = await getPairTradingConfig(
    ...
    true,
    (parseFloat(rfqReq.lPrice) * parseFloat(rfqReq.lQuantity)) /
  ...
  const configParametersShort = await getPairTradingConfig(
    ...
    false,
    (parseFloat(rfqReq.sPrice) * parseFloat(rfqReq.sQuantity)) /
  ...
  if (
    parseFloat(rfqReq.sInterestRate) !== (configParametersShort.funding || 0) ||
  ...
  if (
    parseFloat(rfqReq.lInterestRate) !== (configParametersLong.funding || 0) ||
...
async function fetchBidAskData(assetId: string): Promise<{
  bid: number;
  ask: number;
  qty: number;
  freeHedgerMargin: number;
...
export const processRFQ = async (job: Job) => {
  try {
