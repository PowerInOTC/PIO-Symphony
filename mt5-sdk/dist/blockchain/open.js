...
async function test() {
    ...
    gasLimit = await blockchainInterface.estimateGasLimit(blockchain_client_1.contracts.FakeUSD.name, blockchain_client_1.contracts.FakeUSD.functions.mint, ethers_1.ethers.parseUnits('10000', 18));
    if (!gasLimit) {
        return;
    }
    init_1.logger.info(gasLimit);
    const mint = await blockchainInterface.mint(ethers_1.ethers.parseUnits('10000', 18), gasLimit, gasPrice);
    init_1.logger.info(mint);
    /*
